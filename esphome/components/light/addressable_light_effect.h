...
class AddressableLightEffect : public LightEffect {
 public:
  ...
  void start_internal() override {
    ...
    for (auto view : this->pixels_())
      view.set_effect_data(0);
    this->start();
  }
  void stop() override { this->get_addressable_()->set_effect_active(false); }
  virtual void apply(ESPRangeView &it, const Color &current_color) = 0;
  void apply() override {
    Color current_color = color_from_light_values(this->state_->remote_values);
    ESPRangeView pixels = this->pixels_();
    this->apply(pixels, current_color);
  }

 protected:
  AddressableLight *get_addressable_() const { return (AddressableLight *) this->state_->get_output(); }
  ESPRangeView pixels_() const { return this->get_addressable_()->pixels(); }
  void schedule_show_() const { this->get_addressable_()->schedule_show(); }
};
...
class AddressableLambdaLightEffect : public AddressableLightEffect {
 public:
  ...
  void apply(ESPRangeView &it, const Color &current_color) override {
    const uint32_t now = millis();
    if (now - this->last_run_ >= this->update_interval_) {
      this->last_run_ = now;
      this->f_(*this->get_addressable_(), current_color, this->initial_run_);
...
class AddressableFireworksEffect : public AddressableLightEffect {
 public:
  ...
  void start() override { this->pixels_() = Color::BLACK; }
  void apply(ESPRangeView &it, const Color &current_color) override {
    const uint32_t now = millis();
