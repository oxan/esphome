...
LIGHT_CONTROL_ACTION_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_COLOR): cv.templatable(cv.hex_int_range(0x000000, 0xFFFFFF)),
...
async def light_control_to_code(config, action_id, template_arg, args):
    ...
    if CONF_COLOR in config:
        template_ = await cg.templatable(config[CONF_COLOR], args, cg.uint32)
        cg.add(var.set_color(template_))
